    ...
    while(splitFound) {
        ...
        _.forEach(results,function split(o) {
            ...
            if (!splitFound) {
                ...
                // BUG: splitFound was set by an earlier event in this same pass.
                // This event is not itself split, but is silently dropped because
                // the guard `!splitFound` is false. It should use a per-event flag.
                if (typeof __appendToFile === 'function') {
                    __appendToFile('temp.txt', '[BUG-splitTimespan] dropped event: rate=' + o.rate + ' started_at=' + (o.started_at ? o.started_at.toISOString() : 'null') + ' duration=' + o.duration + 'min');
                }
            }
...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    if (typeof __appendToFile === 'function') {
        __appendToFile('temp.txt', '[calcTempTreatments-buggy] tempHistory.length=' + tempHistory.length + ' splitHistoryByBasal.length=' + splitHistoryByBasal.length + ' clock=' + (now ? now.toISOString() : 'null'));
    }
    ...
    if (typeof __appendToFile === 'function') {
        __appendToFile('temp.txt', '[calcTempTreatments-buggy] tempBoluses.length=' + tempBoluses.length + ' clock=' + (now ? now.toISOString() : 'null'));
    }
