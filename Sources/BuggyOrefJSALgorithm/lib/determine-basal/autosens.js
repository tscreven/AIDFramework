...
//console.error("skipping:",glucose_data[i].glucose,glucose_data[i-1].glucose);
    ...
    for (i=3; i < bucketed_data.length; ++i) {
        ...
        if (typeof __appendToFile === 'function') {
            __appendToFile('temp.txt', '[autosens-iob] t=' + bgTime.toISOString() + ' iob=' + iob.iob + ' activity=' + iob.activity + ' bgiRaw=' + (-iob.activity * sens * 5).toFixed(4) + ' delta=' + delta + ' avgDelta=' + avgDelta);
        }
        ...
        if (typeof __appendToFile === 'function') {
            __appendToFile('temp.txt', '[autosens-deviation] t=' + bgTime.toISOString() + ' bgi=' + bgi + ' delta=' + delta + ' deviation=' + deviation + ' iob=' + iob.iob + ' mealCOB=' + mealCOB);
        }
    ...
    if (typeof __appendToFile === 'function') {
        __appendToFile('temp.txt', 'Buggy autosens: ratio=' + ratio + ' rawRatio=' + rawRatio + ' newisf=' + newisf + ' sens=' + profile.sens + ' pSensitive=' + pSensitive + ' pResistant=' + pResistant + ' basalOff=' + basalOff + ' deviations=' + deviations.length);
    }
    return {
        "ratio": ratio,
        "newisf": newisf
