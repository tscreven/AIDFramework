...
function generate (inputs, opts) {
  ...
  // Use clock from inputs if provided, otherwise default to current time
  var clock = inputs.clock ? new Date(inputs.clock) : new Date();
  ...
  profile.current_basal = basal.basalLookup(inputs.basals, clock);
  ...
  var range = targets.bgTargetsLookup(inputs, profile, clock);
  ...
  profile.sens = isf.isfLookup(inputs.isf, clock);
  ...
  if (typeof(inputs.carbratio) !== "undefined") {
    profile.carb_ratio = carb_ratios.carbRatioLookup(inputs, profile, clock);
