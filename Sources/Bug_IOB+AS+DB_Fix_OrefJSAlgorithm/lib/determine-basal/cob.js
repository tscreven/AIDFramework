...
function detectCarbAbsorption(inputs) {
    ...
    // FIX: exits when there is no glucose data
    if (!glucose_data || glucose_data.length == 0) {
        return null;
    }
    
    // FIX: creates a deep copy of glucose_data[0], bucketed_data[0] does not
    // point to same memory location as glucose_data[0]
...
//console.error(glucose_data[i].glucose, bgTime, Math.round(hoursAfterMeal*100)/100, bucketed_data[bucketed_data.length-1].display_time);
    ...
    //console.error(bgTime, lastbgTime, elapsed_minutes);
        if(Math.abs(elapsed_minutes) > 8) {
            ...
            j++;
            // FIX: creates a deep copy of glucose_data[i], bucketed_data[i]
            // does not point to same memory location as glucose_data[0]
